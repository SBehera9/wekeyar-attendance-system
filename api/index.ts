...
// Storage paths (Vercel uses /tmp for serverless)
...
// Helper functions
function loadData(file: string, defaultValue: any) {
...
if (!users.find((u: any) => u.employeeId === "admin")) users.push(defaultAdmin);
if (!users.find((u: any) => u.employeeId === "hr")) users.push(defaultHR);
if (users.length <= 2) {
...
let locations = loadData('locations.json', [
...
saveData('locations.json', locations);

let attendance = loadData('attendance.json', []);
let leaves = loadData('leaves.json', []);
let auditLogs = loadData('audit_logs.json', []);
let sessionsData = loadData('sessions.json', {});
...
app.post("/api/auth/login", (req, res) => {
  ...
  saveData('sessions.json', Object.fromEntries(activeSessions));
